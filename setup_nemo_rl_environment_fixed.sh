...
# Setup script to replicate NeMo RL Docker environment (FIXED VERSION)
# Based on docker/Dockerfile.ngc_pytorch
# Usage: ./setup_nemo_rl_environment_fixed.sh
...
echo "=== Setting up NeMo RL Environment (Fixed Version) ==="
...
# FIX: Install built vLLM wheel using --python option to specify the virtual environment
...
# FIX: Install NeMo RL dependencies using --python option
...
python -c "import vllm; print('vLLM successfully installed:', vllm.__version__)" || echo "vLLM import might fail due to missing CUDA dependencies - this is expected in some environments"
